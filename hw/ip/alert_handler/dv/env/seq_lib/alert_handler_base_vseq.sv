...
class alert_handler_base_vseq extends cip_base_vseq #(
  ...
  virtual function bit check_esc_done(esc_state_e vals[$]);
    foreach (vals[i]) begin
      if (vals[i] inside {EscStatePhase0, EscStatePhase1, EscStatePhase2, EscStatePhase3}) return 0;
    end
    return 1;
  endfunction

  virtual task wait_esc_handshake_done();
    esc_state_e csr_vals[4];
    do begin
